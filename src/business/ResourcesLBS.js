...
const checkRoomResourceBE = resourceBE => {
	...
	if (!resourceBE.name) {
		...
		if (resourceBE.name.length > MAX_NAME_LENGTH) {
			errors.push(new ErrorDO('name','resource.name.length','La longueur du nom ne doit pas dépasser 100 caractères.'))
		}
	}
	// type
	if (!resourceBE.type) {
	...
	if (!resourceBE.nbSeatsAvailable) {
		errors.push(new ErrorDO('nbSeatsAvailable','resource.nbSeatsAvailable.required','Le nombre de places est obligatoire.'))
	} else {
		if (!Number.isInteger(resourceBE.nbSeatsAvailable)) {
			errors.push(new ErrorDO('nbSeatsAvailable','resource.nbSeatsAvailable.invalid','Le nombre de places doit être un entier.'))
		} else {
			if (resourceBE.nbSeatsAvailable > MAX_SEATS_NUMBER) {
				errors.push(new ErrorDO('nbSeatsAvailable','resource.nbSeatsAvailable.max','Le nombre de places doit inférieur à ' + MAX_SEATS_NUMBER))
			}
		}
	}
	// projectorAvailable
	if (typeof resourceBE.projectorAvailable != 'boolean') {
		errors.push(new ErrorDO('projectorAvailable','resource.projectorAvailable.invalid','L\'indicateur de disponibilité de projecteur est incorrect.'))
	}
	// chromeboxAvailable
	if (typeof resourceBE.chromeboxAvailable != 'boolean') {
		errors.push(new ErrorDO('chromeboxAvailable','resource.chromeboxAvailable.invalid','L\'indicateur de disponibilité de chromebox est incorrect.'))
	}
